## Workspace（原 Datastream）下載資料 須知

解決痛點：由於 Workspace（原 Datastream）有下載限制，無法一次輸出多國多年資料，甚至有些變數太多的國家同一年也沒辦法一次載下來，所以需要分段下載後，再利用本工具做資料整合。
...
Workspace（原 Datastream）下載資料時，要確保每個國家各年度的變數，最後併起來應一致；盡量一個國家一個年份一組變數一組公司就是一個 Excel 檔，不僅下載資料時不容易下錯，也比較不會產生例外狀況。雖然本工具有處理「同一國家多年的變數合併」，但如果你要合併的資料涉及公司群合併（`entity-integrate.py`），目前僅支援「同一國家同一年的公司群合併」。
...
2. **多個變數合併前，檢查分變數檔有沒有少公司（`find-missing-entity-before-variable-integrate.py`）**
...
1. 【分年 + 分變數】（存放於 `./data-split-by-variable`）合併時，理想上同一國家同一年不同的分變數檔 Rows 數應一致，但實務上 Workspace 輸出時可能有些微差異。  
   ...
   **解法：**  
   以 Type 欄位為 Primary Key 做欄位合併，不要無腦將新的欄位向右加
